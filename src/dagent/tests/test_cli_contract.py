...
"""State-based tests for dagent CLI contract and safety gates."""
...
def test_help_includes_examples(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "dagent apply target/dagent.plan.json --yes" in output
...
def test_plan_writes_artifact_and_apply_succeeds(
    ...
    out_plan = tmp_path / "dagent.plan.json"
