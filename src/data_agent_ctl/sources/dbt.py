...
"""dbt manifest source adapter for data-agent-ctl specs."""
...
from data_agent_ctl.diagnostics import Diagnostic, DiagnosticLevel
...
class DbtSourceAdapter:
    ...
    def validate(self) -> list[Diagnostic]:
        ...
        for exposure_name, exposure in exposures:
            ...
            instructions = (
                meta.get("data_agent_ctl_instructions")
            ...
            if not instructions:
                diagnostics.append(
                    Diagnostic(
                        ...
                        message=(
                            f"Exposure missing data-agent-ctl instructions (exposure: {exposure_name})."
                        ),
                        level=DiagnosticLevel.ERROR,
                        path=str(path),
                    )
...
def _to_spec(
    ...
    if not isinstance(meta, dict):
        meta = {}
    data_agent_ctl_meta = meta.get("data-agent-ctl", meta.get("dagent", {}))
    if not isinstance(data_agent_ctl_meta, dict):
        data_agent_ctl_meta = {}

    instructions = (
        data_agent_ctl_meta.get("instructions")
        or meta.get("data_agent_ctl_instructions")
        or meta.get("dagent_instructions")
        or exposure.get("description")
    ...
    iam_mode = _parse_iam_mode(data_agent_ctl_meta.get("iam_mode", IAMMode.IGNORE.value))
    iam_bindings = _parse_iam_bindings(data_agent_ctl_meta.get("iam_bindings", []))
    return DataAgentSpec(
        name=str(data_agent_ctl_meta.get("name", exposure_name)),
        project_id=str(data_agent_ctl_meta.get("project_id", default_project_id)),
        location=str(data_agent_ctl_meta.get("location", default_location)),
        instructions=str(instructions),
        display_name=_opt_text(data_agent_ctl_meta.get("display_name", exposure.get("label"))),
        description=_opt_text(exposure.get("description")),
        labels=labels,
        tags=tags,
        iam_mode=iam_mode,
        iam_bindings=iam_bindings,
        disabled=bool(data_agent_ctl_meta.get("disabled", False)),
        source_metadata={
