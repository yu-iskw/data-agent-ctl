...
"""CLI entry point for data-agent-ctl."""
...
from data_agent_ctl.metrics import METRICS
from data_agent_ctl.models import IAMBinding
from data_agent_ctl.plan.engine import build_plan
from data_agent_ctl.plan_schema import Plan, load_plan, save_plan
from data_agent_ctl.policy import PolicyResult, evaluate_plan, evaluate_specs, load_policy_config
from data_agent_ctl.providers.google_gemini_data_analytics import GoogleGeminiDataAnalyticsProvider
from data_agent_ctl.sources.dbt import DbtSourceAdapter
...
def _build_parser() -> ArgumentParser:  # noqa: PLR0915
    parser = ArgumentParser(
        ...
        epilog=(
            ...
            "  data-agent-ctl plan --source dbt --manifest target/manifest.json "
            "--projects analytics-dev --out target/data-agent-ctl.plan.json\n"
            "  data-agent-ctl apply target/data-agent-ctl.plan.json --yes\n"
        ),
    ...
    apply_parser = subparsers.add_parser(
        ...
        epilog=(
            ...
            "\n"
            "Error examples:\n"
            "  Missing confirmation bypass: data-agent-ctl apply PLAN --yes\n"
        ),
        formatter_class=RawDescriptionHelpFormatter,
    )
...
def _run_plan(args: Namespace) -> int:
    ...
    _emit_log(
        ...
        details={
            ...
            "next": "data-agent-ctl apply <plan-file> --yes",
        },
    )
...
def _load_apply_plan(args: Namespace) -> Plan | ExitCode:
    ...
    if not args.yes:
        print(
            ...
            f"  data-agent-ctl apply {args.plan_file} --yes",
            file=sys.stderr,
        )
        ...
        if not plan_path.exists():
            print(
                f"Error: plan file does not exist.\n  data-agent-ctl apply {plan_path} --yes",
                file=sys.stderr,
            )
...
def _run_iam_set(args: Namespace) -> int:  # noqa: PLR0911
    if not args.yes:
        print(
            ...
            "  data-agent-ctl iam set --resource <resource> --bindings-file <path> --yes",
            file=sys.stderr,
        )
...
def main(argv: list[str] | None = None) -> int:
    """Run data-agent-ctl CLI and return process exit code."""
